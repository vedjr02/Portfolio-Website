    ...
    >
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="mt-14 md:mt-18 space-y-8 md:space-y-10">
          <p className="font-mono text-[11px] tracking-[0.22em] uppercase text-[var(--color-muted)]">
            Main projects
          </p>
        ...
        <div className="mt-20 md:mt-28">
          <div className="flex items-end justify-between gap-4 mb-8">
            <div>
              <p className="font-mono text-[11px] tracking-[0.22em] uppercase text-[var(--color-muted)] mb-2">
                More work
              </p>
              <h3 className="font-display text-2xl md:text-3xl text-[var(--color-ink)] tracking-tight">
  ...
  return (
    <div className="flex flex-col gap-4 max-w-3xl">
      ...
      <BlurFade inView delay={0.1} direction="up">
        <h2 className="font-display text-[clamp(2.75rem,7vw,5.5rem)] leading-[0.92] text-[var(--color-ink)]">
          {title}
        </h2>
      </BlurFade>
      {subtitle && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
        <div className="lg:col-span-8 p-8 md:p-11 lg:p-12 flex flex-col">
          <div className="flex flex-wrap items-center gap-3 mb-7">
            ...
            {consulting && (
              <span className="neu-inset rounded-full px-3 py-1 font-mono text-[10px] tracking-[0.16em] uppercase text-[var(--color-accent)]">
                Current · SEI
              </span>
            )}
          ...
          <p className="font-mono text-[11px] tracking-[0.18em] uppercase text-[var(--color-muted)] mb-3">
            {project.category}
          </p>
          <h3 className="font-display text-[clamp(2rem,4.2vw,3.4rem)] leading-[1.02] tracking-tight text-[var(--color-ink)] max-w-2xl">
            {project.title}
          </h3>
          <p className="mt-5 text-[var(--color-ink-soft)] text-[15px] md:text-base leading-relaxed max-w-2xl">
            {project.description}
          </p>
          ...
          <div className="mt-auto pt-9 flex flex-wrap gap-3">
            {project.liveUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="neu-btn rounded-full px-5 py-3 text-sm font-semibold text-[var(--color-ink)] hover:text-[var(--color-accent)]"
              >
                Open live demo →
              </a>
            )}
        ...
        <div className="lg:col-span-4 p-8 md:p-10 lg:p-12 flex flex-col justify-between gap-8 border-t lg:border-t-0 lg:border-l border-[var(--color-line)]">
          <div className="flex flex-wrap gap-2">
            {project.tags.map((t) => (
              <span
                key={t}
                className="neu-inset rounded-full px-3 py-1.5 text-[12px] text-[var(--color-ink-soft)]"
              >
                {t}
              </span>
            ))}
          ...
          {project.metric ? (
            <div className="neu-inset rounded-[24px] p-6">
              <div className="font-display text-5xl md:text-6xl text-[var(--color-ink)] tracking-tight">
                <NumberTicker
              ...
              <p className="mt-2 font-mono text-[11px] tracking-[0.14em] uppercase text-[var(--color-muted)]">
                {project.metric.label}
              </p>
            </div>
          ) : (
            <div className="neu-inset rounded-[24px] p-6">
              <p className="font-display text-4xl text-[var(--color-ink)] tracking-tight">
                0{index + 1}
              </p>
              <p className="mt-2 font-mono text-[11px] tracking-[0.14em] uppercase text-[var(--color-muted)]">
                Featured engagement
  ...
  const inner = (
    <>
      <div className="flex items-center justify-between gap-2 mb-4">
        ...
        <span className="font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--color-muted)]">
          {project.period.split("—")[0].trim()}
        </span>
      </div>

      <h4 className="font-display text-xl leading-tight tracking-tight text-[var(--color-ink)]">
        {project.title}
      </h4>
      <p className="mt-2 text-[13px] text-[var(--color-ink-soft)] leading-relaxed line-clamp-3">
        {project.description}
      </p>
...
function StatusPill({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={cn(
          "font-mono tracking-[0.16em] uppercase text-[var(--color-ink-soft)]",
